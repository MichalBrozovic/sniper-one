...
var splitMenu = function splitMenu() {
  ...
  window.requestAnimationFrame(function () {
    ...
    var headerStyle = window.getComputedStyle(headerTop);
    var gap = parseFloat(headerStyle.gap) || 0;
    var siteName = headerTop.querySelector(".site-name-wrapper");
    var navButtons = headerTop.querySelector(".navigation-buttons");

    // 1. SPOČÍTÁME ZABRANÉ MÍSTO
    // U search počítáme fixně 250px, u ostatních reálnou šířku
    var siteNameWidth = (siteName === null || siteName === void 0 ? void 0 : siteName.offsetWidth) || 0;
    var navButtonsWidth = (navButtons === null || navButtons === void 0 ? void 0 : navButtons.offsetWidth) || 0;
    var helperWidth = menuHelper.offsetWidth || 80;
    var searchFixedWidth = 150; // Tvoje fixní rezerva pro vyhledávání

    var occupiedWidth = siteNameWidth + navButtonsWidth + helperWidth + searchFixedWidth;

    // 2. MEZERY (GAPY)
    // Počítáme 4 mezery (mezi logem, hledáním, navigací, helperem a tlačítky)
    var totalGapsWidth = 4 * gap;

    // 3. REÁLNÁ DOSTUPNÁ ŠÍŘE
    var containerWidth = headerTop.clientWidth - parseFloat(headerStyle.paddingLeft) - parseFloat(headerStyle.paddingRight);
    var availableWidth = containerWidth - occupiedWidth - totalGapsWidth + 100;
    menuItems.forEach(function (item) {
      ...
      if (offsetRight > availableWidth) {
        ...
        var _itemInHelper = item.id === "nav-manufacturers" ? menuHelper.querySelector("#nav-manufacturers") : menuHelper.querySelector(".".concat(item.classList[0]));
        if (_itemInHelper) _itemInHelper.classList.add("splitted");
      }
    });
...
var initHomepageSwiper = function initHomepageSwiper() {
  ...
  new window.Swiper("#carousel", {
    ...
    // Cursor jen pokud je co posouvat
    watchSlidesProgress: true,
    speed: 600,
    breakpointsBase: "container",
    slidesPerView: itemCount > 1 ? 1.1 : 1,
    // Na mobilu 1.1 jen pokud je víc slidů
    spaceBetween: 12,
    enabled: itemCount > 1,
    // Vypne Swiper úplně, pokud je jen 1 slide
  ...
  if (itemCount <= 1) {
    $carousel.querySelectorAll(".swiper-button-prev, .swiper-button-next").forEach(function (el) {
    ...
    return _regenerator().w(function (_context3) {
      while (1) switch (_context3.p = _context3.n) {
        ...
        case 2:
          ...
          img = fetchedPerex.querySelector("img");
          if (img) {
    ...
    return _regenerator().w(function (_context4) {
      while (1) switch (_context4.n) {
        ...
        case 2:
          ...
          contentDiv = document.createElement("div");
          contentDiv.className = "category-top-upper--content";
          img = currentPerex.querySelector("img");
          if (img) {
            newImg = img.cloneNode(true);
            upperWrapper.append(newImg);
            parentP = img.closest("p");
